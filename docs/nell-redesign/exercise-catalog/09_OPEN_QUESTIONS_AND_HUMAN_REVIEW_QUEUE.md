...
## HR-11 — Adopt lossless PNG recompression in the import pipeline? (opened 2026-08-23)

- **Affected:** all 20 imported composites; future imports.
- **Evidence:** Pillow `optimize=True` re-save of the six largest assets
  reduced them 24.8–28.7% (4,283,961 → 3,132,851 bytes; ~26.9%) with decoded
  pixels proven identical (size, mode, and full-image difference bounding box
  all equal). Extrapolated saving across ExerciseMedia ≈ 2.7 MB pre-actool.
- **Why human:** the importer currently guarantees generated bytes match the
  approved source SHA-256 byte-for-byte (`E_GENERATED_CHECKSUM` family).
  Optimization introduces a deliberate second transformation step, which
  changes the provenance contract (source → losslessly-recompressed output)
  and belongs to the pipeline owner.
- **Safe default:** tracked assets untouched; probe kept out of the repo;
  strict validation remains green.
- **Exact question:** extend the importer with `--optimize-lossless`
  recording both source and output checksums in the index (schema addition),
  or keep byte-exact provenance?
- **Blocker:** blocks nothing; app-size win only.
